...
def _app():
    folder = os.path.join(os.path.dirname
                               ...
                               r"apps/MouseTester")
    if sys.platform == 'win32':
        return os.path.join(folder, "mousebuttons.exe")
    else:
        return os.path.join(folder, "mousebuttons")
...
class MouseTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if sys.platform == 'win32':
            ...
            self.app = subprocess.Popen("exec " + _app(), shell=True)
            time.sleep(1)
