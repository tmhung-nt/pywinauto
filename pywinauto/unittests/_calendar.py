...
class CalendarWrapperTests(unittest.TestCase):
    ...
    def _can_minimize_rectangle_handle_less_than_zero_values(self):
        expected_rect = self._get_expected_minimized_rectangle()
        rect = self.calendar.calc_min_rectangle(-1, -1, -1, -1)
        self.assertEqual(expected_rect, rect)

    def _can_determine_calendar_is_hit(self):
    ...
    def _can_determine_calendar_background_is_hit(self):
        x = int(self.width / self.CALENDARBK_WIDTH_COEFF)
        y = int(self.height - self.CALENDARBK_HEIGHT_OFFSET)
    ...
    def _can_determine_date_is_hit(self):
    ...
    def _can_determine_next_month_date_is_hit(self):
    ...
    def _can_determine_prev_month_date_is_hit(self):
        x = int(self.width / 16)
    ...
    def _can_determine_nothing_is_hit(self):
        res = self.calendar.hit_test(0, 0)
        self.assertEqual(win32defines.MCHT_NOWHERE, res)

    def _can_determine_top_left_title_corner_is_hit(self):
        x = int(self.width / 16)
    ...
    def _can_determine_title_is_hit(self):
    ...
    def _can_determine_title_background_is_hit(self):
        x = int(self.width / self.TITLEBK_WIDTH_COEFF)
        y = int(self.height / self.TITLEBK_HEIGHT_COEFF)
    ...
    def _can_determine_top_right_title_corner_is_hit(self):
    ...
    # def _can_determine_today_link_is_hit(self):
    ...
    def _can_determine_day_abbreviation_is_hit(self):
    ...
    def _can_determine_week_number_is_hit(self):
    ...
    def _can_set_and_get_first_day_of_week(self):
    ...
    def _can_get_default_scroll_rate(self):
        actual_rate = 1

        self.assertEqual(actual_rate, self.calendar.get_month_delta())

    def _can_set_scroll_rate(self):
        actual_rate = 4
        self.calendar.set_month_delta(actual_rate)

        self.assertEqual(actual_rate, self.calendar.get_month_delta())

    def _should_throw_value_error_when_try_to_set_incorrect_scroll_rate(self):
        self.assertRaises(ValueError, self.calendar.set_month_delta, -1)

    def _can_get_month_range_when_calendars_view_into_month(self):
    ...
    def _can_get_month_range_when_calendars_view_into_years(self):
        self.calendar.set_current_date(2017, 5, 2, 2)
        self.calendar.set_view(win32defines.MCMV_YEAR)
    ...
    def _can_get_month_range_with_include_preceding_and_trailing_months(self):
        self.calendar.set_current_date(2017, 5, 2, 2)
    ...
    def _should_throw_value_error_when_try_to_get_month_range_and_scope_of_range_is_incorrect(self):
